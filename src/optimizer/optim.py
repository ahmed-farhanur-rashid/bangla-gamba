"""
Hybrid Muon + AdamW optimizer factory for BanglaGamba.
...
Spec §4.1: Muon group ≈ 150M params, AdamW group ≈ 49M params.
"""
...
MUON_NAME_SUBSTRINGS = (
    "in_proj.weight", "out_proj.weight",        # Mamba-3
    "q_proj.weight", "k_proj.weight",           # GQA
    "v_proj.weight", "o_proj.weight",           # GQA
    "gate_proj.weight", "up_proj.weight",       # FFN
    "down_proj.weight",                          # FFN
)

# AdamW no-decay patterns
NO_DECAY_PATTERNS = ("bias", "norm", "ln", "embed", "B_bias", "C_bias", "dt_bias", ".D")
...
def build_param_groups(model: nn.Module) -> Tuple[List[torch.Tensor], List[torch.Tensor]]:
    ...
    Muon group: all large 2D dense matmul weights (attention, FFN, Mamba projections).
