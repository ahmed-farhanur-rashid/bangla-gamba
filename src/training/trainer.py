...
Implements the full training loop per spec §5:
- BF16 autocast
- Gradient accumulation
- Z-loss (spec §3): 1e-4 * logsumexp(logits).pow(2).mean()
- Global gradient clipping across both param groups
- Dual optimizer step order: muon → adamw → sched × 2 → zero_grad × 2
- Per-group gradient norm logging (first 300 steps, spec §4.7)
...
class Trainer:
    ...
    def compute_loss(self, logits: torch.Tensor, targets: torch.Tensor) -> tuple:
        ...
        # Z-loss: penalty on logit magnitude (spec §6.2)
    ...
    def compute_grad_norms(self) -> tuple:
        """Compute per-group gradient norms for monitoring (spec §4.7)."""
    ...
    def train(self):
        ...
        while self.global_step < self.total_steps:
            ...
            if micro_step % accum == 0:
                ...
                # Per-group gradient norm monitoring (spec §4.7)
                ...
                # Global gradient clipping across both groups (spec §4.5)
